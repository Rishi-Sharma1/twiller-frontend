...
const Feed = () => {
  ...
  const fetchTweets = async () => {
    try {
      setloading(true);
      const res = await axiosInstance.get("/api/tweets");
      setTweets(res.data);
    } catch (error) {
      console.error(error);
    } finally {
  ...
  const handlenewtweet = (newtweet: any) => {
    ...
    // Check for keywords and fire notification
    if (user?.notificationsEnabled && newtweet.content) {
      const text = newtweet.content.toLowerCase();
      if (text.includes("cricket") || text.includes("science")) {
        if ("Notification" in window && Notification.permission === "granted") {
          new Notification("Keyword Alert: Interesting Tweet!", {
